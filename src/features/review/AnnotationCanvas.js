...
export function AnnotationCanvas({ imageUrl, onShapesChange, activeTool = "select", toolColor = "#FF4444", toolThickness = 2, }) {
    ...
    const handleMouseMove = (e) => {
        if (!isDrawing || !canvasRef.current || !layerRef.current)
            return;
        const rect = canvasRef.current.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const y = e.clientY - rect.top;
    ...
    return (_jsx("div", { className: "relative w-full bg-black/5 rounded-lg overflow-hidden", children: _jsx("canvas", { ref: canvasRef, className: "w-full cursor-crosshair", onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseLeave }) }));
}
