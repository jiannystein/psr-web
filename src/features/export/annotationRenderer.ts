...
export async function renderAnnotationsOntoImage(
  imageBlob: Blob,
  shapes: Shape[]
): Promise<Blob> {
...
function drawShapeOnCanvas(ctx: CanvasRenderingContext2D, shape: Shape, _w: number, _h: number): void {
  ctx.save();
...
function drawArrowOnCanvas(ctx: CanvasRenderingContext2D, arrow: ArrowShape): void {
  const headlen = 15;
