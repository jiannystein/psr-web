export function computeSampleHash(ctx: CanvasRenderingContext2D, width: number, height: number): string {
  const sampleSize = 16;
...
export function compareHashes(hash: string, prevHash?: string): { duplicate: boolean; similarity: number } {
  if (!prevHash) {
